...
def make_eigenvalue_coefficients(
    selected_eigenvalues: np.ndarray,
    scheme: str,
):
    """
    Convert selected NMA eigenvalues into coefficient magnitudes.

    Important:
        Eigenvalues only define magnitudes/flexibilities, not signs.
        The signs are added later from the PC·NM projection so the resulting
        combination points as consistently as possible with the selected PC.

    Available schemes:
        inverse_sqrt : coefficient_i = 1 / sqrt(lambda_i)
                       Approximate RMS thermal amplitude scaling.
        inverse      : coefficient_i = 1 / lambda_i
                       Stronger weighting of softer modes.
        sqrt         : coefficient_i = sqrt(lambda_i)
        eigenvalue   : coefficient_i = lambda_i
    """

    selected_eigenvalues = np.asarray(selected_eigenvalues, dtype=float).reshape(-1)

    if selected_eigenvalues.ndim != 1:
        raise ValueError("selected_eigenvalues must be one-dimensional")

    if np.any(~np.isfinite(selected_eigenvalues)):
        raise ValueError(f"NMA eigenvalues contain non-finite values: {selected_eigenvalues}")

    if scheme in {"inverse_sqrt", "inverse"} and np.any(selected_eigenvalues <= 0.0):
        raise ValueError(
            f"Cannot use {scheme} eigenvalue scaling because some selected "
            f"NMA eigenvalues are <= 0:\n{selected_eigenvalues}\n"
            "This usually means you included rigid-body modes. Use --mode-start 6 "
            "or higher for Bio3D internal modes."
        )

    if scheme == "inverse_sqrt":
        coef_mag = 1.0 / np.sqrt(selected_eigenvalues)
    elif scheme == "inverse":
        coef_mag = 1.0 / selected_eigenvalues
    elif scheme == "sqrt":
        coef_mag = np.sqrt(np.maximum(selected_eigenvalues, 0.0))
    elif scheme == "eigenvalue":
        coef_mag = selected_eigenvalues.copy()
    else:
        raise ValueError(f"Unknown eigenvalue coefficient scheme: {scheme}")

    norm = np.linalg.norm(coef_mag)
    if norm == 0.0:
        raise ValueError("Eigenvalue-derived coefficient vector has zero norm.")

    # Normalise coefficients so the combined direction is independent of scale.
    return coef_mag / norm


def project_pc_onto_nma_modes_with_eigenvalue_coefficients(
    pc_vector: np.ndarray,
    nma_modes: np.ndarray,
    nma_eigenvalues_all: np.ndarray,
    mode_start: int,
    n_modes: int,
    scheme: str,
):
    """
    Build a normal-mode combination where coefficient magnitudes come from
    the corresponding NMA eigenvalues.

    The coefficient signs are taken from dot(PC, NM_i), because eigenvalues
    themselves do not contain sign/direction information.
    """

    pc_vector = normalize_vector(np.asarray(pc_vector, dtype=float).reshape(-1))
    nma_modes = np.asarray(nma_modes, dtype=float)
    nma_eigenvalues_all = np.asarray(nma_eigenvalues_all, dtype=float).reshape(-1)

    if nma_modes.ndim != 2:
        raise ValueError(f"Expected nma_modes to be 2D, got shape {nma_modes.shape}")

    expected_dof = pc_vector.shape[0]
    if nma_modes.shape[0] != expected_dof:
        raise ValueError(
            "NMA mode matrix does not match PC vector:\n"
            f"  NMA rows:  {nma_modes.shape[0]}\n"
            f"  PC length: {expected_dof}"
        )

    mode_stop = mode_start + n_modes
    if mode_start < 0:
        raise ValueError("mode_start must be >= 0")
    if mode_stop > nma_modes.shape[1]:
        raise ValueError(
            f"Requested modes [{mode_start}:{mode_stop}], but nma_modes only has "
            f"{nma_modes.shape[1]} columns."
        )
    if mode_stop > nma_eigenvalues_all.shape[0]:
        raise ValueError(
            f"Requested eigenvalues [{mode_start}:{mode_stop}], but "
            f"eigenvalues_all.npy only has {nma_eigenvalues_all.shape[0]} values."
        )

    mode_indices_zero_based = np.arange(mode_start, mode_stop, dtype=int)
    bio3d_mode_numbers = mode_indices_zero_based + 1

    selected_modes = nma_modes[:, mode_indices_zero_based]
    selected_eigenvalues = nma_eigenvalues_all[mode_indices_zero_based]

    # Normalise each mode vector before linear combination.
    selected_modes_norm = np.column_stack([
        normalize_vector(selected_modes[:, i])
        for i in range(selected_modes.shape[1])
    ])

    coef_mag = make_eigenvalue_coefficients(
        selected_eigenvalues=selected_eigenvalues,
        scheme=scheme,
    )

    # Sign cannot come from eigenvalues. Use PC projection signs.
    projection_signs = np.sign(selected_modes_norm.T @ pc_vector)
    projection_signs[projection_signs == 0.0] = 1.0

    coef = coef_mag * projection_signs

    dr_dir = selected_modes_norm @ coef
    dr_dir = normalize_vector(dr_dir)

    # This is no longer sum(coef**2) from orthogonal projection.
    # It is the squared cosine overlap between the PC and the eigenvalue-weighted NM direction.
    pc_nm_overlap = float(np.dot(pc_vector, dr_dir))
    captured = pc_nm_overlap ** 2

    return {
        "coef": coef,
        "coef_magnitudes_from_eigenvalues": coef_mag,
        "projection_signs": projection_signs,
        "selected_nma_eigenvalues": selected_eigenvalues,
        "dr_dir": dr_dir,
        "captured": captured,
        "pc_nm_overlap": pc_nm_overlap,
        "bio3d_mode_numbers": bio3d_mode_numbers,
        "mode_indices_zero_based": mode_indices_zero_based,
        "coef_scheme": scheme,
    }
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "NMA modes using coefficients derived from NMA eigenvalues, and save endpoint structures."
        )
    )
    ...
    parser.add_argument(
        "--eigenvalue-coef-scheme",
        choices=["inverse_sqrt", "inverse", "sqrt", "eigenvalue"],
        default="inverse_sqrt",
        help=(
            "How to convert selected NMA eigenvalues into coefficient magnitudes. "
            "Default inverse_sqrt means coefficient_i ∝ 1/sqrt(lambda_i). "
            "Signs are taken from PC·NM_i because eigenvalues do not define signs."
        ),
    )
    ...
    if args.outdir is None:
        outdir = (
            ...
            / pdb_code
            / f"replica_{replica}"
            / f"pc{pc_index + 1}_nma_projection_test"
        )
    else:
    ...
    print(f"Coef scheme:       {args.eigenvalue_coef_scheme}")
    ...
    if not eigvals_all_path.exists():
        raise FileNotFoundError(
            "This eigenvalue-coefficient version needs NMA eigenvalues:\n"
            f"  missing file: {eigvals_all_path}"
        )

    nma_eigenvalues_all = np.load(eigvals_all_path).reshape(-1)

    print(f"  NMA eigenvalues:  {nma_eigenvalues_all.shape}")

    # ------------------------------------------------------------
    # 4. Build eigenvalue-weighted NMA combination
    # ------------------------------------------------------------
    print("")
    print("[Projection] Building NMA combination with eigenvalue-derived coefficients...")

    result = project_pc_onto_nma_modes_with_eigenvalue_coefficients(
        pc_vector=pc_vector,
        nma_modes=nma_modes,
        nma_eigenvalues_all=nma_eigenvalues_all,
        mode_start=args.mode_start,
        n_modes=args.n_modes,
        scheme=args.eigenvalue_coef_scheme,
    )
    ...
    np.save(outdir / "coef_magnitudes_from_eigenvalues.npy", result["coef_magnitudes_from_eigenvalues"])
    np.save(outdir / "projection_signs.npy", result["projection_signs"])
    np.save(outdir / "selected_nma_eigenvalues.npy", result["selected_nma_eigenvalues"])
    np.save(outdir / "pc_nm_overlap.npy", np.array([result["pc_nm_overlap"]]))
    ...
    np.save(outdir / "nma_eigenvalues_all.npy", nma_eigenvalues_all)
    ...
    print(f"Coef scheme:        {result['coef_scheme']}")
    print(f"NMA eigenvalues:    {result['selected_nma_eigenvalues']}")
    print(f"Coefficients:       {result['coef']}")
    print(f"PC-NM overlap:      {result['pc_nm_overlap']:.6f}")
    print(f"Captured/overlap^2: {result['captured']:.6f}")
    print(f"Amplitude:          {amplitude}")
    print(f"Amplitude source:   {amplitude_source}")
