...
Replica-overlap PCA atomic displacement plots (Bio3D-like per-residue PC contributions)

This script:
  - loads PCA eigenvectors (and eigenvalues if --weighted) for multiple replicas
  - projects eigenvectors onto Cα coordinates using traj_utils.build_protein_heavy_views()
  - computes per-residue contributions: ||v_k,i|| (or sqrt(lambda_k)*||v_k,i|| if --weighted)
  - makes OVERLAY plots: each replica as a thin line + mean as a thick line (+ optional SD/SEM band)

Filesystem logic supported:
  A) Manual mode:
     --eigdir results/1a7u/rep0_pca_eigs [results/1a7u/rep1_pca_eigs ...]

  B) Auto-discovery mode:
     --eigroot results/1a7u
       -> finds results/1a7u/rep*_pca_eigs (customisable via --eigglob)

Outputs:
  - one PNG containing panels for selected PCs with replica overlays
...
def pc_contribution(V_ca: np.ndarray, k: int) -> np.ndarray:
    """
    Per-residue contribution for PC k (after CA projection):
...
def pc_contribution_weighted(V_ca: np.ndarray, lambdas: np.ndarray, k: int) -> np.ndarray:
    """
    1-sigma amplitude scaling:
      c_i = sqrt(lambda_k) * || v_k,i ||
    """
    return np.sqrt(float(lambdas[k])) * pc_contribution(V_ca, k)
...
def plot_replica_overlay_panels(
    stacks: list[np.ndarray],          # each: (Nrep, Nres)
    means: list[np.ndarray],           # each: (Nres,)
    spreads: list[np.ndarray] | None,  # each: (Nres,) or None
    pc_labels: list[str],
    res_start: int,
    out_png: Path,
    y_label: str,
    ...
    for i, (stack, mean, label) in enumerate(zip(stacks, means, pc_labels)):
        row, col = divmod(i, n_cols)
        ax = axes[row][col]

        x = np.arange(res_start, res_start + stack.shape[1])

        # Replica lines
        for r in range(stack.shape[0]):
            ax.plot(x, stack[r], linewidth=1, alpha=0.35)
        ...
        if spreads is not None:
            s = spreads[i]
            ax.fill_between(x, mean - s, mean + s, alpha=0.20, label=band_label)
...
def main() -> None:
    ...
    ap.add_argument("--band", choices=["std", "sem"], default="std", help="Band type (std or sem).")
    ap.add_argument("--no-band", action="store_true", help="Disable SD/SEM band shading (overlay lines + mean only).")
    ...
    pc_labels: list[str] = [f"PC{p} (√λ scaled)" if args.weighted else f"PC{p}" for p in pcs]
    ...
    for eigdir in eigdirs:
        ...
        if n_atoms_V != n_atoms_top:
            raise ValueError(
                ...
                f"  eigenvectors imply Natoms={n_atoms_V}\n"
                f"  topology has         Natoms={n_atoms_top}\n"
            )

        if args.weighted and lambdas is None:
            raise FileNotFoundError(f"[{eigdir}] Requested --weighted but eigenvalues.npy was not found")

        V_ca = slice_eigenvectors_to_ca(V_all, ca_idx)

        K = V_ca.shape[0]
        if max(pcs) > K:
            raise ValueError(f"[{eigdir}] Requested PC{max(pcs)} but only {K} components are available")

        for p in pcs:
            k = p - 1
            if args.weighted:
                c = pc_contribution_weighted(V_ca, lambdas, k)  # type: ignore[arg-type]
            else:
                c = pc_contribution(V_ca, k)
            per_pc_rep_contribs[p].append(c)

    # Build stacks/means/spreads per PC
    ...
    spreads_arg = None if args.no_band else spreads

    out_png = Path(args.out)
    plot_replica_overlay_panels(
        stacks=stacks,
        means=means,
        spreads=spreads_arg,
        pc_labels=pc_labels,
    ...
    print(f"Replicas: {len(eigdirs)} | PCs: {pcs} | band: {'none' if args.no_band else args.band}")
    print("Eigdirs used:")
    for p in eigdirs:
        print(f"  - {p}")
